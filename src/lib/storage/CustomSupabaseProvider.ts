...
export class CustomSupabaseProvider implements RemoteStorageProvider {
  ...
  async upsertFinanceEntity(entityType: string, payload: unknown): Promise<void> {
    const mapping = getCustomFinanceTableAndRow(entityType, payload);
    if (!mapping) return;
    const sb = this.client();
  ...
  async deleteFinanceEntity(entityType: string, id: string): Promise<void> {
    const table = getCustomFinanceTable(entityType);
    if (!table) return;
    const sb = this.client();
...
function getCustomFinanceTable(entityType: string): string | null {
...
function getCustomFinanceTableAndRow(entityType: string, payload: unknown): { table: string; row: Record<string, unknown> } | null {
